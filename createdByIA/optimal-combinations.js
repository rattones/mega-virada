...
const top10Numbers = Array.from(numberFrequency.entries())
    .sort((a, b) => b[1] - a[1])
    .slice(0, 10)
    .map(([num]) => num);
...
function findBestEightNumbers(top10Numbers) {
    let bestCombination = null;
    let maxNewCombinations = 0;

    // Generate all possible combinations of 8 numbers from top 10
    const eightNumberSets = generateCombinations(top10Numbers, 8);
...
const bestResult = findBestEightNumbers(top10Numbers);

console.log('Os 10 números mais sorteados:');
console.log(top10Numbers.join(', '));
